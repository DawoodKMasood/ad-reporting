...
router
  .group(() => {
    ...
    router
      .get('/accounts', async ({ auth, response }) => {
        try {
    ...
    router
      .post('/accounts/:id/switch', async ({ params, auth, response }) => {
        try {
          const user = auth.getUserOrFail()
          ...
          return response.json({
            ...
            account: {
              ...
              formattedAccountId:
                connectedAccount.accountId.length === 10
    ...
    router
      .get('/accessible-customers', [IntegrationsController, 'getAccessibleCustomers'])
      .as('integrations.accessible_customers')
    ...
    router
      .post('/connect/:platform', [IntegrationsController, 'connect'])
      .as('integrations.connect.post')

    router
      .get('/callback/:platform', [IntegrationsController, 'callback'])
      .as('integrations.callback')

    router
      .post('/disconnect/:id', [IntegrationsController, 'disconnect'])
      .as('integrations.disconnect')

    router.post('/sync/:id', [IntegrationsController, 'sync']).as('integrations.sync')

    router
      .get('/:id/sync-status', [IntegrationsController, 'getSyncStatus'])
      .as('integrations.sync_status')

    router
      .patch('/:id/name', [IntegrationsController, 'updateAccountName'])
      .as('integrations.update_name')
